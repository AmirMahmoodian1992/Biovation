...
namespace Biovation.Server.Controllers.v2
{
    ...
    public class UserGroupController : ControllerBase
    {
        ...
        [HttpPatch]
        [Route("{id:int}/UserGroupMember")]
        public async Task<ResultViewModel> ModifyUserGroupMember([FromRoute] int id, [FromBody] List<UserGroupMember> members)
        {
            try
            {
                var token = HttpContext.Items["Token"] as string;
                ...
                var usersToDelete = existingUserGroup.Users.ExceptBy(members, member => member?.UserId).ToList() ?? new List<UserGroupMember>();

                var usersToAdd = (members.ExceptBy(existingUserGroup.Users, member => member.UserId).ToList()) ?? new List<UserGroupMember>();
                ...
                var result = await _userGroupService.ModifyUserGroup(existingUserGroup, token);
                if (result.Validate != 1) return result;
