...
namespace Biovation.Services.RelayController.Commands
{
    public class Contact : ICommand
    {
        ...
        public ResultViewModel Execute()
        {
            ...
            foreach (var relayScheduling in Relay.RelayInfo.Schedulings)
            {
                if (DateTime.Now.TimeOfDay >= relayScheduling.StartTime & DateTime.Now.TimeOfDay <= relayScheduling.EndTime & relayScheduling.Mode.Name == "Close")
                    if (_priority.Code != TaskPriorities.ImmediateCode)
                        return new ResultViewModel { Validate = 0, Success = false, Message = $"Relay Id: {Relay.RelayInfo.Id} Contact failed !.\nthe command conflicts with the scheduling with scheduling ID: {relayScheduling.Id}.", Code = 1, Id = Relay.RelayInfo.Id };
            }
