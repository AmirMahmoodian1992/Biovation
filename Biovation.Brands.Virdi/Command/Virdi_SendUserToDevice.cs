    ...
    public class VirdiSendUserToDevice : ICommand
    {
        ...
        public object Execute()
        {
            ...
            try
            {
                ...
                _serverUserData.IsBlacklist = UserObj.IsActive ? IsBlackList : 1;
                ...
                _serverUserData.UserID = (int)UserObj.Code;
                _serverUserData.UniqueID = Math.Abs(UserObj.UniqueId).ToString();
                _serverUserData.UserName = userName;

                if (UserObj.ImageBytes != null && UserObj.ImageBytes.Length > 0)
                    _serverUserData.SetPictureData(UserObj.ImageBytes.Length, "JPG", UserObj.ImageBytes);

                var adminDevices = _adminDeviceService.GetAdminDevicesByUserId(personId: UserId);
                _serverUserData.IsAdmin = adminDevices.Any(x => x.DeviceId == DeviceId) ? 1 : 0;

                _serverUserData.IsIdentify = UserObj.IsActive ? 1 : 0;
                // Set Access Flag
                _serverUserData.IsFace1toN = 0;
                ...
                if (activeCard != null)
                {
                    _serverUserData.SetCardData(1, activeCard.CardNum);
                    isCard = true;
                }
                ...
                if (!string.IsNullOrEmpty(UserObj.Password))
                {
                    try
                    {
                        ...
                        _serverUserData.Password = UserObj.Password;
                        isPassword = true;
                    }
                    catch (Exception e)
                    {
                ...
                if (virdiFinger.Count != 0)
                {
                    ...
                    {
                        _serverUserData.AddFingerData(virdiFinger[i].FingerIndex.OrderIndex, (int)UCBioAPI.Type.TEMPLATE_TYPE.SIZE400, virdiFinger[i].Template, virdiFinger[i + 1].Template);
                    }
                ...
                if (virdiFace != null)
                {
                    _serverUserData.FaceNumber = virdiFace.Index;
                    _serverUserData.FaceData = virdiFace.Template;
                    _serverUserData.IsFace1toN = UserObj.IsActive ? 1 : 0;
                    isFace = true;
                }
                ...
                if (vWthFace != null && vWthFace.Size > 0)
                {
                    ...
                    _serverUserData.SetWalkThroughData(int.Parse(dataType), vWthFace.Size, vWthFace.Template);
                    _serverUserData.IsFace1toN = UserObj.IsActive ? 1 : 0;
                    isFace = true;
                }
                ...
                {
                    _serverUserData.SetIrisData(irisData.Size, irisData.Template);
                    _serverUserData.IsIris1toN = UserObj.IsActive ? 1 : 0;
                    isIris = true;
                }
                ...
                {
                    _serverUserData.AuthType = (int)AuthType.CardOrFpOrFace;
                }
                else if (isCard && isPassword)
                {
                    _serverUserData.AuthType = (int)AuthType.CardOrPassword;
                }
                else if ((isFace || isIris) && isPassword)
                {
                    _serverUserData.AuthType = (int)AuthType.FaceOrPassword;
                }
                else if (isFingerPrint && isPassword)
                {
                    _serverUserData.AuthType = (int)AuthType.FpOrPassword;
                }
                else if ((isFace || isIris) && isFingerPrint)
                {
                    _serverUserData.AuthType = (int)AuthType.FpOrFace;
                }
                else if (isCard && isFingerPrint)
                {
                    _serverUserData.AuthType = (int)AuthType.CardOrFp;
                }
                else if (isCard && (isFace || isIris))
                {
                    _serverUserData.AuthType = (int)AuthType.CardOrFace;
                }
                else if ((isFace || isIris))
                {
                    _serverUserData.AuthType = (int)AuthType.Face;
                }
                else if (isFingerPrint)
                {
                    _serverUserData.AuthType = (int)AuthType.Fp;
                }
                else if (isCard)
                {
                    _serverUserData.AuthType = (int)AuthType.Card;
                }
                else if (isPassword)
                {
                    _serverUserData.AuthType = (int)AuthType.Password;
                }

                _serverUserData.SetAuthType(Convert.ToInt32(false), Convert.ToInt32(isFingerPrint),
                    Convert.ToInt32(false), Convert.ToInt32(isPassword),
                    Convert.ToInt32(isCard), Convert.ToInt32(isIris));

                _serverUserData.SetAuthTypeEx(Convert.ToInt32(isFace), 0, 0, Convert.ToInt32(isIris), 0, 0, 0, 0);
                ...
                _serverUserData.AccessGroup = validAccessGroup?.Id.ToString("D4") ?? "****";

                if (UserObj.EndDate != default && UserObj.StartDate != default && UserObj.StartDate < UserObj.EndDate)
                    _serverUserData.SetAccessDate(1, UserObj.StartDate.Year, UserObj.StartDate.Month, UserObj.StartDate.Day, UserObj.EndDate.Year, UserObj.EndDate.Month, UserObj.EndDate.Day);

                _serverUserData.AddUserToTerminal(TaskItemId, (int)Code, 1);

                Logger.Log("-->Add user to terminal");

                if (_serverUserData.ErrorCode == 0)
                {
                ...
                Logger.Log($"  +Cannot transfer user {UserId} to device: {Code}. Error code = {_serverUserData.ErrorCode}\n");
                return new ResultViewModel { Code = Convert.ToInt64(TaskStatuses.FailedCode), Id = DeviceId, Message = $"  +Cannot transfer user {UserId} to device: {Code}. Error code = {_serverUserData.ErrorCode}\n", Validate = 1 };
            }
            catch (Exception ex)
            {
