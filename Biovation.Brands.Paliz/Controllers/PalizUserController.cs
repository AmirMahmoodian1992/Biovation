...
namespace Biovation.Brands.Paliz.Controllers
{
    ...
    public class PalizUserController : ControllerBase
    {
        ...
        public PalizUserController(TaskService taskService, DeviceService deviceService, AccessGroupService accessGroupService, DeviceBrands deviceBrands, TaskTypes taskTypes, TaskStatuses taskStatuses, TaskItemTypes taskItemTypes, TaskPriorities taskPriorities)
        {
        ...
        public async Task<List<ResultViewModel>> SendUserToDevice(uint code, string userId, bool updateServerSideIdentification = false)
        {
            ...
            try
            {
                ...
                _taskService.InsertTask(task);
                await _taskService.ProcessQueue(_deviceBrands.Paliz, device.DeviceId).ConfigureAwait(false);
        ...
        public async Task<ResultViewModel> DeleteUserFromTerminal(uint code, int userId)
        {
            return await Task.Run(() =>
            {
                ...
                return new ResultViewModel { Id = userId, Validate = 1 /*Convert.ToInt32(result)*/ };
            });
        }
        ...
        public async Task<ResultViewModel> EnrollFaceTemplate(int userId, int deviceId)
        {
            try
            {
                ...
                _taskService.InsertTask(task);
                await _taskService.ProcessQueue(_deviceBrands.Paliz, deviceId).ConfigureAwait(false);
