    ...
    public class ZkLogService
    {
        ...
        public Task<ResultViewModel> AddLog(Log log)
        {
            return Task.Run(async () =>
            {
                ...
                */
                var device = _commonDeviceService.GetDeviceBasicInfoWithCode(log.DeviceCode, DeviceBrands.ZkTecoCode);
                log.InOutMode = device?.DeviceTypeId ?? 0;
                var addLogResult = await _commonLogService.AddLog(log);
                //Thread.Sleep(200);
                return addLogResult;
            });
        }
            ...
            return Task.Run(async () =>
            {
                try
                {
                   /*ar logsAuthenticationTypes = logs.GroupBy(g => g.MatchingType).Select(s => s.Key).ToList();
                    ...
                    {
                        var device = _commonDeviceService.GetDeviceBasicInfoWithCode(deviceCode, DeviceBrands.ZkTecoCode);
                        ...
                        await Task.Run(async () =>
                        {
                            ...
                            foreach (var log in logsWithImages)
                            {
                                var filePath = log.PicByte is null
                                    ? string.Empty
                                    : await _commonLogService.SaveImage(log.PicByte, log.UserId, log.LogDateTime, log.DeviceCode, DeviceBrands.ZkTeco.Name);
                                log.Image = filePath;
