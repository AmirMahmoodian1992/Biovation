    ...
    public class SupremaRetrieveUserFromDevice : ICommand
    {
        ...
        public object Execute()
        {
            ...
            if (userOfDevice is null)
                ...
                { Id = DeviceId, Message = "0", Validate = 1, Code = Convert.ToInt64(TaskStatuses.DoneCode) };
            ...
            try
            {
                ...
                var existUser = _userService.GetUsers(code: userOfDevice.Code).GetAwaiter().GetResult()?.Data?.Data.FirstOrDefault();
                ...
                user.Id = _userService.GetUsers(code: userOfDevice.Code).GetAwaiter().GetResult()?.Data?.Data.FirstOrDefault()?.Id ?? 0;
                ...
                try
                {
                    if (!(userOfDevice.FingerTemplates is null))
                    {
                        ...
                        for (var i = 0; i < nFpDataCount; i += 2)
                        {
                            ...
                            if (existUser != null)
                            {
                                ...
                                var fingerTemplates = existUser.FingerTemplates
                                    .Where(ft => ft.FingerTemplateType.Code == _fingerTemplateTypes.SU384.Code)
                                    .ToList();
                                ...
                                user.FingerTemplates.Add(new FingerTemplate
                                {
                                    ...
                                    Index = _fingerTemplateService.FingerTemplates((int)existUser.Id).GetAwaiter().GetResult()?.Data.Data.Count(
                                        ft =>
                                            ft.FingerIndex.Code ==
                            ...
                            else
                            {
                                ...
                                user.FingerTemplates.Add(new FingerTemplate
                                {
                                    FingerIndex = userOfDevice.FingerTemplates[i].FingerIndex,
                ...
                try
                {
                    if (!(userOfDevice.FaceTemplates is null))
                    {
                        ...
                        if (faceCount > 0)
                        {
                            ...
                            if (existUser?.FaceTemplates != null)
                            {
                                if (!existUser.FaceTemplates.Exists(fp =>
                                    fp.FaceTemplateType.Code == _faceTemplateTypes.SFACE.Code))
                ...
                //            {
                //                var onlineDevices =
                //                    _deviceService.GetDevices(brandId: DeviceBrands.SupremaCode)?.Data.Data;
