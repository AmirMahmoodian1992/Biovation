...
namespace Biovation.Brands.PW.Service
{
    public class PwLogService
    {
        private readonly LogService _logService;
        private readonly DeviceService _deviceService;

        public PwLogService(LogService logService, DeviceService deviceService)
        {
        ...
        public Task<ResultViewModel> AddLog(Log log)
        {
            return Task.Run(async () =>
            {
                ...
                //log.MatchingType = authMode?.Id ?? 0;
        ...
        public Task<ResultViewModel> AddLog(List<Log> logs)
        {
            return Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        var device = _deviceService.GetDevices(code: deviceCode, brandId: DeviceBrands.ProcessingWorldCode)?.FirstOrDefault();
                    ...
                    foreach (var deviceCode in deviceCodes)
                    {
                        ...
                        //        log.EventId == newLog.EventId && log.DeviceId == newLog.DeviceId) && log.PicByte?.Length > 0).ToList();

                        //    foreach (var log in logsWithImages)
                        //    {
                        //        var filePath = log.PicByte is null
                        //            ? string.Empty
                        //            : await _commonLogService.SaveImage(log.PicByte, log.UserId, log.LogDateTime, log.DeviceCode, DeviceBrands.ProcessingWorld.Name);
                        //        log.Image = filePath;
